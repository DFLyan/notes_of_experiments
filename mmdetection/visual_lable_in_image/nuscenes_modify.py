...
class NuScenes_modify(NuScenes):
    def __init__(self, version: str = 'v1.0-mini',
                 dataroot: str = '/data/sets',
                 verbose: bool = True,
                 map_resolution: float = 0.1):
        super(NuScenes_modify, self).__init__(version=version,
                                              dataroot=dataroot,
                                              verbose=verbose,
                                              map_resolution=map_resolution)
        self.explorer = NuScenesExplorer_modify(self)
    ...
    def _render_sample_data(self, sample_data_token: str, with_anns: bool = True,
        ...
        self.explorer._render_sample_data(sample_data_token, with_anns, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
    ...
    def _render_sample_data_bbox(self, sample_data_token: str, with_anns: bool = True,
                           box_vis_level: BoxVisibility = BoxVisibility.ANY, axes_limit: float = 40, ax: Axes = None,
                           nsweeps: int = 1, out_path: str = None, underlay_map: bool = True,
                           use_flat_vehicle_coordinates: bool = True,
                           show_lidarseg: bool = False,
                           ...
                           filter_lidarseg_labels: List = None,
                           lidarseg_preds_bin_path: str = None, verbose: bool = True) -> None:
        self.explorer._render_sample_data_bbox(sample_data_token, with_anns, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
                                         out_path=out_path, underlay_map=underlay_map,
                                         use_flat_vehicle_coordinates=use_flat_vehicle_coordinates,
                                         show_lidarseg=show_lidarseg,
                                         show_lidarseg_legend=show_lidarseg_legend,
...
class NuScenesExplorer_modify(NuScenesExplorer):
    def _render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            for box in boxes:
                ...
                ## From the higher-left corner, draw a line in a clockwise direction
                ax.plot([min_x, max_x], [min_y, min_y], color='b', linewidth=2)
                ax.plot([max_x, max_x], [min_y, max_y], color='b', linewidth=2)
                ax.plot([max_x, min_x], [max_y, max_y], color='b', linewidth=2)
                ax.plot([min_x, min_x], [max_y, min_y], color='b', linewidth=2)
            # plt.show()

            # using 3D box(eight points).
            # if with_anns:
            #     for box in boxes:
            #         c = np.array(self.get_color(box.name)) / 255.0
    ...
    def _render_sample_data_bbox(self,
                           sample_data_token: str,
                           with_anns: bool = True,
                           box_vis_level: BoxVisibility = BoxVisibility.ANY,
                           axes_limit: float = 40,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            # Show image.
            # ax.imshow(data, cmap=mpl.colors.ListedColormap([(0, 0, 0, 0)]))
            # ax.imshow(data)
